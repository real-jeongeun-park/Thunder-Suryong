...
public class AiService {
    ...
    private String buildPrompt(Map<String, Object> body){
        ...
        if(!style.equals("")){
            ...
            sb.append("❗ 3. **출력/응답 시 반드시 (YYYY-MM-DD 형식의 날짜, 과목 이름, 공부 분량) 이라는 형식을 지켜. 형식을 지키지 않으면 계획표는 저장되지 않아.** **앞뒤로, 혹은 중간에 다른 설명, 요약, 문장은 절대 포함 시키지마.**");
            ...
            sb.append("❗ 주의: 다시 한 번 말하지만, **출력/응답 시 반드시 (YYYY-MM-DD 형식의 날짜, 과목 이름, 공부 분량) 이라는 형식을 지켜. 형식을 지키지 않으면 계획표는 저장되지 않아.** **앞뒤로, 혹은 중간에 다른 설명, 요약, 문장은 절대 포함 시키지마.**");
            sb.append("❗ 주의: 다시 한 번 말하지만, **출력/응답 시 반드시 (YYYY-MM-DD 형식의 날짜, 과목 이름, 공부 분량) 이라는 형식을 지켜. 형식을 지키지 않으면 계획표는 저장되지 않아.** **앞뒤로, 혹은 중간에 다른 설명, 요약, 문장은 절대 포함 시키지마.**");
        } else {
            sb.append("너는 똑똑한 AI 공부 도우미야. 요구사항에 따라 학습 계획표를 세워줘.\n");
            sb.append("[요구사항]\n");
